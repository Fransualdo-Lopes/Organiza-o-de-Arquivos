...
dtype = {str(i): str for i in range(10)}  
arquivos = carregar_dados('arquivos.csv', dtype=dtype)
            ...
            if cliente_id in clientes['Id_cli'].values:
                ...
                if not arquivos_atendimento.empty:
                    ...
                    for _, arquivo in tqdm(arquivos_atendimento.iterrows(), total=len(arquivos_atendimento), desc=f"Copiando arquivos para {pasta_atendimento}", leave=False):
                        caminho_arquivo_origem = os.path.join(pasta_origem, str(arquivo['NomeArquivo']))
                        caminho_arquivo_destino = os.path.join(pasta_atendimento, os.path.basename(str(arquivo['NomeArquivo'])))
